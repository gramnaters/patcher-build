...
def process_download(downloaded_path: str) -> None:
    """Inspect downloaded file: if it's a ZIP/XAPK/APKS bundle, extract base.apk
    + arm64 split. Otherwise treat as a standalone APK."""
    with open(downloaded_path, "rb") as f:
        magic = f.read(4)
    if magic != b"PK\x03\x04":
        # Not a ZIP — assume it's already a standalone APK
    ...
    if has_manifest:
        ...
        log("XAPK/APKS bundle detected — extracting base.apk")
        with zipfile.ZipFile(downloaded_path, "r") as z:
            z.extract("base.apk", ".")
            shutil.move("base.apk", OUTPUT_APK)
            # Extract arm64 split if present
            arm64_split = None
            for name in names:
                if "arm64" in name.lower() and name.endswith(".apk"):
                    ...
                    break
            if arm64_split:
                log(f"Found arm64 split: {arm64_split}")
                z.extract(arm64_split, ".")
                if os.path.exists(arm64_split):
                    shutil.move(arm64_split, "arm64_split.apk")
            else:
                log("No arm64 split found in bundle")
    ...
    log(f"Ready: {OUTPUT_APK} ({final_mb:.1f} MB)")
